...
class TestLinearRegression extends FlatSpec {
  ...
  "An linear regression" should "learn from inputs and outputs" in {
    ...
    linearRegress.train()
    assert(linearRegress.forecast(-1.0) >= -2.30 && linearRegress.forecast(-1.0) <= -1.70)
    assert(linearRegress.forecast(20.0) >= 39.70 && linearRegress.forecast(20.0) <= 40.30)
    assert(linearRegress.forecast(70.0) >= 139.70 && linearRegress.forecast(20.0) <= 140.30)
  }
